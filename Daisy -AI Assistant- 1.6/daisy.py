#!/usr/bin/env python3
"""
Daisy Assistant 1.6 - Main CLI entrypoint
...
class DaisyPipeline:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        # 1.3: long-term memory store + conversation summarizer
        from services.memory import MemoryStore, ConversationSummarizer
        memory_db_path = Path(self.config.paths.database_path).expanduser().parent / "memory.db"
        self.memory_store = MemoryStore(memory_db_path)
        self.summarizer = ConversationSummarizer(
            max_turns=getattr(self.config, "max_conversation_history", 30),
            compress_n=10,
            brain_service=self.brain_service,
        )

        # 1.4: undo stack — 1.6: persisted so "undo" survives launchd restarts
        from services.undo_stack import UndoStack, UndoManager
        undo_path = Path(self.config.paths.database_path).expanduser().parent / "undo.json"
        self.undo_stack = UndoStack(persist_path=undo_path)
        self.undo_manager = UndoManager(self.config, memory_store=self.memory_store)
        ...
        # 1.6: serialize turns — process_text mutates conversation_history,
        # the summarizer, sqlite and the undo stack; concurrent HTTP/WS
        # callers would interleave that state.
        self._turn_lock = threading.Lock()

        # 1.6: clean shutdown — stop the scheduler thread and flush sqlite
        # WAL instead of relying on daemon threads dying with the process.
        import atexit
        atexit.register(self.shutdown)

    def shutdown(self) -> None:
        """Stop background threads and close resources. Safe to call twice."""
        try:
    ...
    def _push_undo(self, action, result=None) -> None:
        """Push an undo entry for a successfully executed action (1.4).

        1.6: uses the exact side-effect details from ActionResult.metadata
        (note path, reminder created_at) instead of guessing — the old
        mtime-glob heuristic could delete the wrong note when two titles
        shared a prefix.
        """
        try:
            from services.undo_stack import UndoEntry
            at = action.action_type
            result_meta = getattr(result, "metadata", None) or {}

            if at == "create_note" and action.create_note:
                path = result_meta.get("path", "")
                if not path:
                    # Fallback for callers that didn't pass the result.
                    from pathlib import Path as _Path
                    notes_dir = _Path(self.config.paths.notes_directory).expanduser()
                    safe = "".join(c for c in action.create_note.title
                                   if c.isalnum() or c in (' ', '-', '_')).strip().replace(' ', '_') or "untitled"
                    candidates = sorted(notes_dir.glob(f"{safe}*.md"), key=lambda p: p.stat().st_mtime, reverse=True)
                    path = str(candidates[0]) if candidates else ""
                self.undo_stack.push(UndoEntry(
                    action_type="create_note",
                    description=f"Create note: {action.create_note.title}",
                    metadata={"path": path},
                ))

            elif at == "create_task" and action.create_task:
                self.undo_stack.push(UndoEntry(
                    action_type="create_task",
                    description=f"Create task: {action.create_task.title}",
                    metadata={"title": action.create_task.title},
                ))

            elif at == "create_reminder" and action.create_reminder:
                self.undo_stack.push(UndoEntry(
                    action_type="create_reminder",
                    description=f"Create reminder: {action.create_reminder.message}",
                    metadata={
                        "message": action.create_reminder.message,
                        "created_at": result_meta.get("created_at"),
                    },
                ))

        except Exception as exc:
            logger.warning("_push_undo failed: %s", exc)
    ...
    def process_text(self, text: str, auto_approve: bool = False) -> str:
        """
        Process text input through the pipeline.

        auto_approve: explicit opt-in from a trusted local caller (HTTP API).
        It is passed straight to the dispatcher per-call — never by mutating
        shared state — and every auto-approved confirmation is audit-logged.

        Returns: Response text for TTS
        """
        with self._turn_lock:
            return self._process_text_locked(text, auto_approve=auto_approve)

    def _process_text_locked(self, text: str, auto_approve: bool = False) -> str:
        ...
        # 1.3: compress old turns if history is long
        history_dicts = [{"role": m.role, "content": m.content} for m in self.conversation_history]
        compressed = self.summarizer.maybe_compress(history_dicts)
        compressed_history = [
            ConversationMessage(role=m["role"], content=m["content"])
            for m in compressed
        ]

        # 1.3: inject relevant long-term memories as a system message
        memories = self.memory_store.search(text, limit=3)
        if memories:
            mem_text = "\n".join(f"- {m['topic']}: {m['content']}" for m in memories)
            compressed_history.insert(0, ConversationMessage(
                role="system",
                content=f"Relevant memories from previous sessions:\n{mem_text}",
            ))

        # Brain: Plan actions
        intent = self.brain_service.plan_actions(transcript, compressed_history)
        ...
        for action in intent.actions:
            if action.action_type == "conversation":
                ...
                # 1.6: the LLM's requires_confirmation flag can only ADD a
                # confirmation requirement — it can never bypass per-action
                # rules (the old `auto_approve=not intent.requires_confirmation`
                # silently disabled the whole confirmation layer).
                results = self.dispatcher.dispatch_actions(
                    [action],
                    auto_approve=auto_approve,
                    force_confirmation=intent.requires_confirmation,
                )
                action_results.extend(results)
                # 1.4: push successful reversible actions onto undo stack
                for res in results:
                    if res.success:
                        self._push_undo(action, res)
