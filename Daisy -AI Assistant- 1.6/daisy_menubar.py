...
def main() -> None:
    ...
    class DaisyApp(rumps.App):
        def __init__(self):
            ...
            # 1.6: poll via rumps.Timer (runs on the main AppKit loop).
            # Mutating menu titles from a raw background thread is undefined
            # behavior in AppKit and could freeze or crash the tray.
            self._status_timer = rumps.Timer(self._poll_once, 3)
        ...
        # ---------- status poll (main-loop timer, 1.6) ----------

        def _poll_once(self, _timer=None):
            try:
