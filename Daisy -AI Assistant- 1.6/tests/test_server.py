...
# ---------------------------------------------------------------------------
# 1.4 — Undo / log / settings endpoint smoke tests
# ---------------------------------------------------------------------------

def test_undo_list_empty():
    client = TestClient(create_app())
    r = client.get("/api/undo")
    assert r.status_code == 200
    assert "undo_stack" in r.json()
    assert isinstance(r.json()["undo_stack"], list)


def test_undo_execute_nothing_returns_404():
    client = TestClient(create_app())
...
# ---------------------------------------------------------------------------
# 1.5 — Health check + version history endpoints
# ---------------------------------------------------------------------------
...
def test_version_history_endpoint():
    client = TestClient(create_app())
    r = client.get("/api/version-history")
    assert r.status_code == 200
    data = r.json()
    assert "versions" in data
    versions = data["versions"]
    assert isinstance(versions, list)
    assert len(versions) >= 10  # 0.6 through 1.5
    # Check latest entry
    latest = versions[-1]
    assert latest["version"] == "1.6"
