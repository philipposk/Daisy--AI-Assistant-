...
/**
 * Escapes a value for safe embedding inside an AppleScript string literal.
...
class DesktopAutomationServer {
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              delay: {
                type: "number",
                description: "Delay between keystroke chunks in milliseconds (default: 50)",
              },
            },
            required: ["text"],
          },
  ...
  async takeScreenshot(args) {
    ...
    if (args.windowName) {
      // Screenshot specific window: look up the window id first, then capture it
      const script = `tell application "System Events" to get id of first window whose name contains "${escapeAS(args.windowName)}"`;
      const idResult = await execFileAsync("osascript", ["-e", script]);
      const windowId = idResult.stdout.trim();
      await execFileAsync("screencapture", [`-l${windowId}`, outputPath]);
    } else {
      await execFileAsync("screencapture", [outputPath]);
    }
    return {
      content: [
        {
  ...
  async clickMouse(args) {
    if (args.elementText) {
      ...
      const elementText = escapeAS(args.elementText);
      const script = `
        tell application "System Events"
          set frontApp to first application process whose frontmost is true
          tell frontApp
            try
              set targetButton to first button whose name contains "${elementText}" or value contains "${elementText}" or description contains "${elementText}"
              click targetButton
              return "Clicked element: ${elementText}"
            on error
              return "Could not find element: ${elementText}"
            end try
          end tell
        end tell
      `;
      ...
      const script = `
        tell application "System Events"
          click at {${Number(args.x)}, ${Number(args.y)}}
        end tell
      `;
  ...
  async typeText(args) {
    const delayMs = args.delay !== undefined ? args.delay : 50;
    const delaySeconds = Math.max(0, delayMs) / 1000;

    // Type in chunks, honoring the requested delay between chunks
    const chunkSize = 20;
    ...
    const keystrokeLines = chunks
      .map((chunk) => `keystroke "${escapeAS(chunk)}"`)
      .join(delaySeconds > 0 ? `\n        delay ${delaySeconds}\n        ` : "\n        ");
  ...
  async keyPress(args) {
    ...
    const script = `
      tell application "System Events"
        keystroke "${escapeAS(mainKey)}"${modString}
      end tell
    `;
  ...
  async openApplication(args) {
    const script = `
      tell application "${escapeAS(args.appName)}"
        activate
      end tell
    `;
    await execFileAsync("osascript", ["-e", script]);
  ...
  async findUIElement(args) {
    const text = escapeAS(args.text);
    const script = `
      tell application "System Events"
        set frontApp to first application process whose frontmost is true
        tell frontApp
          try
            set targetElement to first UI element whose name contains "${text}" or value contains "${text}"
            set elementPosition to position of targetElement
            set elementSize to size of targetElement
            return "Found element: ${text} at " & (item 1 of elementPosition) & "," & (item 2 of elementPosition) & " size: " & (item 1 of elementSize) & "x" & (item 2 of elementSize)
          on error
  ...
  async runTerminalCommand(args) {
    const timeout = args.timeout !== undefined ? args.timeout : 60_000;
    const maxBuffer = args.maxBuffer !== undefined ? args.maxBuffer : 4 * 1024 * 1024;
    const options = { timeout, maxBuffer };
    if (args.workingDirectory) {
      options.cwd = args.workingDirectory;
    }
    ...
    try {
      ...
      if (error.killed && error.signal === "SIGTERM") {
        throw new Error(
          `Command timed out after ${timeout / 1000}s: ${args.command}`
        );
      }
      throw error;
    }
