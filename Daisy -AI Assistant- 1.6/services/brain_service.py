...
6. create_calendar_event - User wants an event in macOS Calendar
   - Extract: title, start_iso, end_iso (both ISO 8601 datetimes, e.g. "2026-06-10T15:00:00"), optional calendar_name, optional notes

7. create_mac_reminder - User wants an item in macOS Reminders
   - Extract: title, optional due_iso (ISO 8601 datetime), optional list_name, optional notes

8. send_email - User wants to send (or draft) an email via Mail.app
   - Extract: to, subject, body, optional cc, draft_only (true → create a draft instead of sending)

9. conversation - General conversation, question answering, or no specific action needed
...
Output format (JSON):
{
  "actions": [
    {
      "action_type": "create_note|create_task|create_reminder|run_command|mcp_tool_call|create_calendar_event|create_mac_reminder|send_email|conversation",
...
class BrainService:
    ...
    def __init__(self, config: Config):
        ...
        # 1.6: circuit breaker + cost tracking across the provider chain.
        from services.provider_health import CircuitBreaker, CostTracker
        from pathlib import Path as _Path
        _data_dir = _Path(getattr(config.paths, "database_path", "~/.daisy/daisy.db")).expanduser().parent
        self.circuit_breaker = CircuitBreaker()
        self.cost_tracker = CostTracker(persist_path=_data_dir / "costs.json")
...
Analyze this request and determine what action(s) to take. Output valid JSON only."""
        ...
        # 1.6: skip providers whose circuit breaker is open, track cost).
        provider_chain = self._build_provider_chain()
        response_text = None
        last_err: Optional[Exception] = None
        for provider in provider_chain:
            if not self.circuit_breaker.allow(provider):
                logger.info("Skipping '%s' — circuit breaker open.", provider)
                continue
            try:
    ...
    # 1.6: "free" = llm-free-rotator (the author's PyPI package) — rotates
    # across free-tier providers (Groq / OpenRouter / NVIDIA) on rate limits.
    # Optional dependency; the provider is only offered when importable.
    _DEFAULT_PROVIDER_ORDER = ("openai", "anthropic", "groq", "free", "local_http")
    ...
    def _provider_available(self, provider: str) -> bool:
        ...
        if provider == "free":
            return self._free_rotator_available()
        return False
    ...
    def _call_provider(self, provider: str, prompt: str) -> str:
        ...
        if provider == "free" and self._free_rotator_available():
            return self._call_free_rotator(prompt)
    ...
    # ----------------- 1.6: llm-free-rotator -----------------

    _free_rotator = None  # lazily constructed, cached on the instance

    def _free_rotator_available(self) -> bool:
        try:
            import llm_free_rotator  # noqa: F401
            return True
        except ImportError:
            return False

    def _call_free_rotator(self, prompt: str) -> str:
        """Route through llm-free-rotator (free-tier rotation across
        OpenRouter / Groq / NVIDIA with automatic 429/5xx fallback).

        Install with: pip install llm-free-rotator
        Provider API keys are picked up from the environment by the package.
        """
        from llm_free_rotator import FreeRotator
        if self._free_rotator is None:
            self._free_rotator = FreeRotator()
        result = self._free_rotator.chat(
            messages=[
                {"role": "system", "content": _current_system_prompt()
                 + "\n\nIMPORTANT: You MUST respond with valid JSON only. "
                   "Output format: {\"actions\": [...], \"requires_confirmation\": false}"},
                {"role": "user", "content": prompt},
            ],
            max_tokens=self.config.llm.max_tokens,
            temperature=self.config.llm.temperature,
        )
        # Package returns either a plain string or an OpenAI-shaped response.
    ...
    def _extract_json(self, text: str) -> dict:
        ...
        try:
            ...
            # Don't echo raw model/provider output back to the user — it may be
            # an error message or other text never meant to be spoken aloud.
            logger.warning(f"Could not extract JSON from: {text[:100]}...")
            return {
                "actions": [{
                    "action_type": "conversation",
                    "conversation": "Sorry — I didn't catch that properly. Could you rephrase?",
                }]
            }
    
    def _parse_actions(self, response_json: dict) -> List[AssistantAction]:
        """Parse actions from JSON response"""
        from schemas import CreateNoteAction, CreateTaskAction, CreateReminderAction, RunCommandAction, MCPToolCallAction
        from schemas.models import CreateCalendarEventAction, CreateMacReminderAction, SendEmailAction
        ...
        for action_data in response_json.get("actions", []):
            ...
            try:
                if action_type == "create_note":
                    ...
                    mcp_data = action_data.get("mcp_tool_call", {})
                    mcp_action = MCPToolCallAction(**mcp_data)
                    actions.append(AssistantAction(
                        action_type="mcp_tool_call",
                        mcp_tool_call=mcp_action,
                        confidence=action_data.get("confidence"),
                        reasoning=action_data.get("reasoning"),
                    ))
                elif action_type == "create_calendar_event":
                    cal_data = action_data.get("create_calendar_event", {})
                    cal_action = CreateCalendarEventAction(**cal_data)
                    actions.append(AssistantAction(
                        action_type="create_calendar_event",
                        create_calendar_event=cal_action,
                        confidence=action_data.get("confidence"),
                        reasoning=action_data.get("reasoning"),
                    ))
                elif action_type == "create_mac_reminder":
                    mac_reminder_data = action_data.get("create_mac_reminder", {})
                    mac_reminder_action = CreateMacReminderAction(**mac_reminder_data)
                    actions.append(AssistantAction(
                        action_type="create_mac_reminder",
                        create_mac_reminder=mac_reminder_action,
                        confidence=action_data.get("confidence"),
                        reasoning=action_data.get("reasoning"),
                    ))
                elif action_type == "send_email":
                    email_data = action_data.get("send_email", {})
                    email_action = SendEmailAction(**email_data)
                    actions.append(AssistantAction(
