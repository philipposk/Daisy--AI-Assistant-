...
from services.transcript_filter import is_probably_real
...
class VoiceService:
    ...
    def transcribe_audio_file(self, audio_path: Path) -> TranscriptionResult:
        ...
        0.9 wraps the result with a hallucination filter (transcript_filter.is_probably_real).
        Phantoms ("thanks for watching", bare "you", etc.) are blanked.
        """
        result = self._transcribe_audio_file_inner(audio_path)
        if not is_probably_real(result.text, getattr(result, "confidence", None)):
            logger.info(f"Hallucination filter dropped transcript: {result.text!r}")
            return TranscriptionResult(text="", language=result.language,
                                       confidence=result.confidence,
                                       segments=result.segments)
        return result

    def _transcribe_audio_file_inner(self, audio_path: Path) -> TranscriptionResult:
        # Try primary provider
        try:
            if self.config.stt.provider == "realtimestt":
                return self._transcribe_realtimestt(audio_path)
            elif self.config.stt.provider == "openai" and self.openai_client:
    ...
    def _transcribe_realtimestt(self, audio_path: Path) -> TranscriptionResult:
        """
        Transcribe via faster-whisper directly (RealtimeSTT's STT path,
        without the full streaming live-mic loop — we already have a WAV).

        Optional dep: `pip install faster-whisper`. Tries CoreML on Apple
        Silicon if `whisperkit` is available; else CPU/CUDA.
        """
        try:
            ...
            raise RuntimeError(
                ...
                "or set stt.provider to 'openai'/'google'."
            ) from e

        model_size = getattr(self.config.stt, "local_model", None) or "base.en"
        # Reuse a single model instance across calls
        if not getattr(self, "_fw_model", None):
            self._fw_model = WhisperModel(model_size, device="auto", compute_type="auto")

        segments_iter, info = self._fw_model.transcribe(str(audio_path), beam_size=1, vad_filter=True)
        from schemas import TranscriptionSegment
        segments = []
        full_text_parts = []
        for i, seg in enumerate(segments_iter, 1):
            segments.append(TranscriptionSegment(
                id=i, start=seg.start, end=seg.end, text=(seg.text or "").strip(),
            ))
    ...
    def text_to_speech(self, text: str, output_path: Optional[Path] = None) -> Path:
        ...
        try:
            ...
            elif self.config.tts.provider == "kokoro":
                return self._tts_kokoro(text, output_path)
            else:
    ...
    def _tts_kokoro(self, text: str, output_path: Path) -> Path:
        """
        Kokoro TTS (0.9). Apache-2.0 model + weights. ~1-2s time-to-first-audio
        on Mac CPU. Streaming-friendly but here we do one-shot synthesis.

        Optional dep: `pip install kokoro-onnx`. Falls back if missing.
        Config (added to TTSConfig):
            tts.kokoro_model     # path to kokoro .onnx
            tts.kokoro_voices    # path to voices npz/json
            tts.kokoro_voice     # voice id, e.g. "af_sarah"
        """
        try:
        ...
        model = getattr(self.config.tts, "kokoro_model", None)
        voices = getattr(self.config.tts, "kokoro_voices", None)
        voice = getattr(self.config.tts, "kokoro_voice", None) or "af_sarah"
        if not model or not voices:
            raise RuntimeError(
                "Kokoro requires tts.kokoro_model and tts.kokoro_voices paths. "
                "Download from https://github.com/thewh1teagle/kokoro-onnx"
            )

        if not getattr(self, "_kokoro", None):
            self._kokoro = Kokoro(str(Path(model).expanduser()), str(Path(voices).expanduser()))
    ...
    def play_audio(self, audio_path: Path, wait: bool = True, track: bool = True) -> None:
        """
        Play audio file.
        ...
        track=False skips storing the handle on self.current_playback, so
        side-channel audio (e.g. reminder TTS) can't clobber the barge-in handle
        for the main speech that's currently playing.
        """
        try:
            ...
            try:
                ...
                logger.warning(
                    "No audio player found (tried afplay, aplay); skipping playback"
                )
                return None
        ...
        if wait:
            try:
                ...
                if track and self.current_playback is proc:
