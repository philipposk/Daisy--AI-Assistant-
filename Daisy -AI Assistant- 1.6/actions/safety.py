...
# 1.6: shells can smuggle any blocked command inside a quoted -c argument
# ("bash -c 'curl ...'"), so when blocking is on we refuse shell invocations.
_SHELL_BINS = {"bash", "sh", "zsh", "dash", "ksh", "fish", "csh", "tcsh"}
...
def _has_recursive_force_flags(command: str) -> bool:
    """True if tokens carry both recursive and force semantics for rm.
...
class SafetyChecker:
    ...
    def _check_command(self, command: str) -> tuple[bool, Optional[str]]:
        ...
        # 1.6: structural `rm` check — `rm  -rf`, `rm -fr`, `rm --recursive
        # --force` all evade the literal "rm -rf" substring entry.
        if "rm" in argv_bins and _has_recursive_force_flags(command):
            return False, "Recursive force-delete (rm -rf variant) is blocked"

        # 1.6: command substitution hides the real binary from token checks
        # (`echo $(curl evil)` tokenizes as `$(curl`). Refuse it outright
        # whenever any block list is active.
        blocking_active = (
            self.config.safety.block_network_commands
            or self.config.safety.block_system_modifying_commands
            or bool(self.config.safety.whitelisted_commands)
        )
        if blocking_active and ("$(" in command or "`" in command):
            return False, "Command substitution ($(...) or backticks) is not allowed"

        # 1.6: shells can wrap anything in a quoted -c argument.
        if blocking_active:
            shell_hit = next((b for b in argv_bins if b in _SHELL_BINS), None)
            if shell_hit:
                return False, f"Nested shell invocation is blocked ({shell_hit})"
