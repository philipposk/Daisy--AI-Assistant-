...
# 1.6: single source of truth — launchd_setup, menubar and the docs all use
# 5188; this entrypoint used to default to 8765 and the menubar then showed
# "backend offline" for anyone who ran `python3 daisy_app.py` bare.
DEFAULT_PORT = 5188

# Held so the main thread can ask uvicorn to exit cleanly (1.6: the daemon
# thread used to be killed mid-write when the window closed).
...
def _shutdown_server(server_thread: threading.Thread) -> None:
    """Ask uvicorn to exit and give it a moment to finish in-flight work."""
...
def _open_window(url: str) -> None:
    ...
    webbrowser.open(url)
    print(f"Daisy UI opened in your default browser: {url}")
...
def main() -> None:
    ...
    try:
        if args.no_ui:
            print(f"Daisy backend running at {url} (no window).")
            print("Press Ctrl+C to stop.")
            try:
                while True:
                    time.sleep(3600)
