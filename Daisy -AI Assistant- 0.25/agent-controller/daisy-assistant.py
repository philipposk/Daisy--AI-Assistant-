...
class DaisyAssistant:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.tts_engine = self.config.get("tts_engine", "piper")  # piper, openai, say
...
Keep responses concise but friendly. Use natural speech patterns."""
        ...
        if tts_engine == "piper":
            ...
            print(f"   ✅ OpenAI TTS: Available (voice: {voice})")
    ...
    def create_default_config(self) -> Dict:
        ...
        config = {
            ...
            "tts_engine": "piper",  # piper (fast, open-source), openai (cloud), say (macOS fallback)
            ...
            "greek_piper_model": "el_GR-rapunzelina-low",  # Greek Piper TTS model
...
with natural rhythm and emphasis.""",
    ...
    def _say_tts_fallback(self, text: str) -> Optional[bytes]:
        """Fallback to macOS say command (interruptible)"""
        try:
            ...
            # Text is already processed, use as-is
            processed_text = text
            ...
            print(f"⚠️  macOS say fallback error: {e}")
            self.is_speaking = False
            self.current_audio_process = None
            return None
