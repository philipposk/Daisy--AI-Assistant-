# Daisy 1.1 — macOS Keychain secrets

Stop encouraging users to paste API keys into shell scripts. Daisy now reads
them from the macOS Keychain at config-load time.

## What's new

- `services/keychain.py` — thin wrapper over the `security` CLI. No new pip
  dependency. CRUD over a single service `daisy` with one entry per key:
  `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GROQ_API_KEY`, `GOOGLE_API_KEY`.
- `config/config_loader.py` — Keychain becomes a third overlay layer between
  the YAML config file and environment variables. Env vars still win over
  Keychain (useful for one-off testing). File config never gets secrets.
- New endpoints:
  - `GET    /api/keychain`          → `{available, known_keys, set_keys}`
  - `POST   /api/keychain`          → body `{name, value}` writes a secret
  - `DELETE /api/keychain/{name}`   → removes a secret

## Tests

- `tests/test_keychain.py` — get/set/delete/list with mocked `subprocess.run`.
- `tests/test_server.py` — new endpoint shape + unknown-key rejection.

## Net effect

Users can stop committing keys. The `0.5`→`0.6` leak that triggered the
filter-repo cleanup wouldn't have happened with this layer.

---

# Daisy 1.0 — Streaming + Menubar + Permissions

First post-prerelease cut. Closes the 2026 table-stakes gap.

## What's new

### 1. Streaming responses (SSE)
- New endpoint `GET /api/turn-stream?text=...` emits Server-Sent Events:
  `transcript` → 1+ `partial` (typewriter chunks) → `done`.
- Frontend uses `EventSource` so the assistant bubble updates live.
- Backend pseudo-streams word-by-word; real LLM token streaming hooks in 1.3.

### 2. Menu-bar tray icon (`daisy_menubar.py`)
- macOS menu-bar app via `rumps`. Run alongside `daisy_app.py`.
- Menu items: Status (live provider chain), Open Daisy…, Permissions…
  (opens System Settings to the right pane), Quit.

### 3. First-run permissions (`services/permissions.py` + endpoints)
- `check_permissions()` reports microphone / accessibility / screen
  recording status (PyAudio probe, AppleScript sentinel, Quartz preflight).
- `open_settings(kind)` deep-links to the right Privacy pane.
- New endpoints `GET /api/permissions`, `GET /api/permissions/open?kind=…`.
- Frontend shows a yellow banner with "Open Settings" when any permission
  is missing.

## Endpoints (new)

```
GET  /api/permissions           -> {microphone, accessibility, screen_recording: bool}
GET  /api/permissions/open?kind=mic|accessibility|screen -> {ok, kind}
GET  /api/turn-stream?text=...  -> SSE: transcript, partial..., done
```

## New tests
- `tests/test_permissions.py`
- `tests/test_server.py` — permissions + SSE coverage

## Optional deps
- `rumps` for the menu bar
- `pyobjc-framework-Quartz` for precise screen-recording check

## Limitations / what's left
- Real LLM token streaming (vs word-chunking) still pending — 1.3.
- Menu-bar + backend lifecycles are independent.
