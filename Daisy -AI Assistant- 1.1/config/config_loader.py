...
class STTConfig:
    ...
    provider: str = "openai"  # openai, google, local_http, realtimestt
    openai_api_key: Optional[str] = None
    google_api_key: Optional[str] = None
    local_http_url: Optional[str] = None  # http://localhost:8000/transcribe
    local_model: Optional[str] = None     # 0.9: faster-whisper model size (e.g. "base.en")
...
class TTSConfig:
    ...
    # 0.9: Kokoro TTS settings (Apache-2.0 neural TTS).
    kokoro_model: Optional[str] = None       # path to kokoro .onnx
    kokoro_voices: Optional[str] = None      # path to voices npz/json
    kokoro_voice: Optional[str] = None       # voice id, e.g. "af_sarah"
...
class WakeWordConfig:
    ...
    enabled: bool = False
    provider: str = "openwakeword"   # openwakeword | whisper_poll | disabled
    keyword: str = "daisy"
    openwakeword_model: Optional[str] = None
...
class ConfirmationConfig:
    """Confirmation-prompt behaviour (0.8)."""
    mode: str = "cli"  # cli | voice | auto_reject | auto_approve
    timeout_seconds: float = 30.0


@dataclass
class MCPConfig:
    """Model Context Protocol servers Daisy can dispatch to (0.8)."""
    desktop_automation_command: Optional[List[str]] = None   # e.g. ["node", "mcp-desktop-automation/server.js"]
    computer_use_command: Optional[List[str]] = None         # e.g. ["python", "mcp-computer-use/server.py"]
...
class Config:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Config":
        ...
        if "wake_word" in data:
            config.wake_word = _safe(WakeWordConfig, data["wake_word"])
        if "reminder" in data:
            config.reminder = _safe(ReminderConfig, data["reminder"])
        if "confirmation" in data:
            config.confirmation = _safe(ConfirmationConfig, data["confirmation"])
        if "mcp" in data:
            config.mcp = _safe(MCPConfig, data["mcp"])
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reminder": self.reminder.__dict__,
...
def load_config(config_path: Optional[Path] = None) -> Config:
    ...
    # 3a. (1.1) Overlay macOS Keychain. Env vars still win over Keychain.
    try:
        import sys as _sys
        from pathlib import Path as _P
        _sys.path.insert(0, str(_P(__file__).resolve().parents[1]))
        from services.keychain import get_secret, KNOWN_KEYS
        kc_overlay: Dict[str, Any] = {}
        for _k in KNOWN_KEYS:
            _v = get_secret(_k)
            if not _v:
                continue
            if _k == "OPENAI_API_KEY":
                kc_overlay.setdefault("llm", {})["openai_api_key"] = _v
                kc_overlay.setdefault("stt", {})["openai_api_key"] = _v
                kc_overlay.setdefault("tts", {})["openai_api_key"] = _v
            elif _k == "ANTHROPIC_API_KEY":
                kc_overlay.setdefault("llm", {})["anthropic_api_key"] = _v
            elif _k == "GROQ_API_KEY":
                kc_overlay.setdefault("llm", {})["groq_api_key"] = _v
            elif _k == "GOOGLE_API_KEY":
                kc_overlay.setdefault("stt", {})["google_api_key"] = _v
        if kc_overlay:
            merged = _deep_merge(merged, kc_overlay)
    except Exception:
        pass  # Keychain failure must never break config load

    # 3b. Overlay environment variables (fix #20: added ANTHROPIC + GOOGLE)
