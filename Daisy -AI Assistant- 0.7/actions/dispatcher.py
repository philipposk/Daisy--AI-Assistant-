...
class ActionDispatcher:
    ...
    def dispatch_actions(self, actions: List[AssistantAction], auto_approve: bool = False) -> List[ActionResult]:
        ...
        for action in actions:
            ...
            # Fix #15: confirmation-required actions now produce an explicit
            # failed result instead of being silently dropped. 0.8 will wire a
            # real ConfirmationProvider; until then, callers can see what was
            # blocked and surface a message to the user.
            if self.safety_checker.requires_confirmation(action) and not auto_approve:
                ...
                results.append(ActionResult(
                    action=action,
                    success=False,
                    error="Confirmation required (not auto-approved). Re-run with confirmation to proceed.",
                ))
                continue
